...
\begin{enumerate}
  ...
  \item \textbf{Open artifact}: The LLM-Sentry pipeline implementation, dataset construction scripts, and a 5,963-package real-world metadata dataset are released at \url{https://github.com/ufxa/Software-Supply-Chain-Security}. The experiment runner (\texttt{a100\_runner.py}) validates the pipeline mechanics end-to-end; full numerical reproduction of the OMB-25 results additionally requires the complete dataset and API access (OpenAI, Docker sandbox).
\end{enumerate}
...
\begin{tikzpicture}[
  ...
  actor/.style={rectangle, draw=blue!60, fill=blue!10, minimum width=2.0cm, minimum height=0.65cm, align=center}
]
...
\foreach \x in {0,\dx,2*\dx,3*\dx,4*\dx,5*\dx}
  \draw[lifeline] (\x,-0.33) -- (\x,-7.5*\dy);

\draw[msg] (0,-1*\dy)   -- (\dx,-1*\dy)     node[midway,above]{\texttt{submit(pkg)}};
\draw[msg] (\dx,-1.7*\dy) -- (2*\dx,-1.7*\dy) node[midway,above]{\texttt{extract\_meta()}};
\draw[msg] (2*\dx,-2.3*\dy) -- (\dx,-2.3*\dy) node[midway,above]{\texttt{meta\_vector}};
\draw[msg] (\dx,-3.0*\dy) -- (3*\dx,-3.0*\dy) node[midway,above]{\texttt{analyze\_code(ast,script)}};
\draw[msg] (3*\dx,-3.7*\dy) -- (\dx,-3.7*\dy) node[midway,above]{\texttt{llm\_confidence}};
\draw[msg] (\dx,-4.4*\dy) -- (4*\dx,-4.4*\dy) node[midway,above]{\texttt{sandbox\_execute()}};
\draw[msg] (4*\dx,-5.1*\dy) -- (\dx,-5.1*\dy) node[midway,above]{\texttt{api\_sequence}};
\draw[msg] (\dx,-5.8*\dy) -- (5*\dx,-5.8*\dy) node[midway,above]{\texttt{compute\_PRCS(m,e,c,b)}};
\draw[msg] (5*\dx,-6.5*\dy) -- (\dx,-6.5*\dy) node[midway,above]{\texttt{PRCS, label}};
\draw[msg] (\dx,-7.2*\dy) -- (0,-7.2*\dy)     node[midway,above]{\texttt{report(PRCS, label)}};
...
\textbf{Proposition 2} (Boundedness and Threshold Existence): $\text{PRCS}(p) \in [0,1]$ for all packages $p$. Moreover, for fixed $\mathbf{w}$, there exists a threshold $\tau^* \in (0,1)$ that minimizes $f(\tau) = \text{FPR}(\tau) + \lambda \cdot \text{FNR}(\tau)$ for any $\lambda > 0$, provided the sub-score distributions of malicious and benign packages overlap.

\textit{Proof}: Boundedness: each $s_k(p) \in [0,1]$ by the sigmoid range, cosine similarity normalization (restricted to $[0,1]$ by construction), and the $\min(1,\cdot)$ clamp in Eq.~(\ref{eq:sbeh}). Since $\sum_k w_k = 1$ and $w_k \geq 0$, PRCS is a convex combination, hence $\text{PRCS}(p) \in [0,1]$. For the threshold: define $f(\tau) = \text{FPR}(\tau) + \lambda \cdot \text{FNR}(\tau)$. As $\tau \to 0$, $\text{FPR} \to 1$ and $\text{FNR} \to 0$, giving $f(0) = 1$; as $\tau \to 1$, $\text{FPR} \to 0$ and $\text{FNR} \to 1$, giving $f(1) = \lambda$. Under distributional overlap, $f$ is continuous and strictly greater than $\min(1, \lambda)$ at the boundary points, so a minimizer $\tau^* \in (0,1)$ exists by the extreme value theorem. $\square$
...
The artifact package is publicly available at \url{https://github.com/ufxa/Software-Supply-Chain-Security}. It contains two categories of artifacts:

\textbf{Pipeline implementation (fully runnable):} (1) \texttt{code/llm\_sentry.py} -- the LLM-Sentry six-stage pipeline; (2) \texttt{code/a100\_runner.py} -- a self-contained GPU experiment runner that executes the complete pipeline end-to-end (all eight stages including CodeBERT embedding, LLM scoring, behavioral analysis, PRCS computation, ablation variants, and cross-ecosystem evaluation) and produces JSON result files structurally identical to those reported in this paper; (3) \texttt{data/real\_dataset\_5963.csv} -- a 5,963-package real-world metadata dataset (2,000 OSSF-confirmed malicious packages and 3,963 benign packages from npm and PyPI) that can be used directly with the runner.

\textbf{Full-scale result reproduction:} (4) \texttt{code/build\_ombb25\_dataset.py} -- the OMB-25 dataset construction script (requires \texttt{GITHUB\_TOKEN} and approximately four hours of API calls); (5) \texttt{code/run\_baselines.py} -- baseline reimplementations; (6) \texttt{code/run\_llm\_analysis.py} -- LLM semantic analysis stage (requires OpenAI API access); (7) \texttt{code/compute\_final\_metrics.py} -- merges all prediction outputs and generates the \LaTeX{} tables in this paper. Reproducing the exact F1 = 0.961 figures from Table~\ref{tab:results_main} requires the complete OMB-25 dataset, an OpenAI API key, and a Docker-capable environment for behavioral sandbox execution.

The runner falls back to a synthetic benchmark mirroring OMB-25 size and class distribution when real dataset CSVs are absent, providing end-to-end verification of the pipeline mechanics without dataset collection.
